...
## Load data
d1 = read.delim('U2OS2-9_output_single.0622Aligned.out.Rdata_clvEffTable.txt') %>% rename_all( ~ paste0(.x, '_KD'))
d2 = read.delim('U2OSNT2_output_single.0622Aligned.out.Rdata_clvEffTable.txt') %>% rename_all( ~ paste0(.x, '_WT'))
...
d1 %>% mutate(start = start_KD - upS_motifDist_KD, 
  ...
  write.table(., 'input_U2OS2-9.bed', sep='\t', quote = F, row.names = F, col.names = F)
...
d2 %>% mutate(start = start_WT - upS_motifDist_WT, 
  ...
  write.table(., 'input_U2OSNT2.bed', sep='\t', quote = F, row.names = F, col.names = F)

## Set up the plot 
stBedFiles <- list("input_U2OS2-9.bed", "input_U2OSNT2.bed")
